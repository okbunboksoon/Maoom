<xsl:stylesheet version="2.0" 
	...
	<xsl:variable name="filename-prefix-parts" select="$map-title-tokens[normalize-space(.) != '' and not(upper-case(.) = ('KIA', 'PE', 'PE2','LHD', 'RHD'))]"/>
...
-->
	...
	<xsl:template match="concept | task | reference">
		...
		<xsl:variable name="chapter-container" select="if (parent::topicref) then ($current-topicref/parent::chapter[parent::map], $current-topicref[parent::map])[1] else ()"/>
		<xsl:variable name="chapter-number" select="if ($chapter-container) then format-number(count($chapter-container/preceding-sibling::*[self::topicref or self::chapter]) + 1, '00') else ''"/>
	    <!-- 부모 topicref href -->
