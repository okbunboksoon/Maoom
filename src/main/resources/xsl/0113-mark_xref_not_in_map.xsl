...
<xsl:stylesheet version="3.0"
    ...
    <!-- xslt 3.0 추가된 코드 원본 복사 -->
    <xsl:mode on-no-match="shallow-copy"/>

    <!-- href에서 # 뒤의 fragment와 앞쪽 경로를 제외한 파일명만 반환 -->
    ...
    <!-- 병합된 map의 topicref가 참조하는 DITA 파일명 목록 -->
    ...
    <!--
        xref의 # 뒤 fragment는 검사하지 않음
        xref가 가리키는 DITA 파일이 topicref 목록에 없을 때만 modified에 표시
    -->
