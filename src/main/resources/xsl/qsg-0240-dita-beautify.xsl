<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:param name="TOPICS_URI" select="'../topics/'"/>
	<xsl:param name="OUT_TOPICS_URI" select="'../topics/'"/>
	<xsl:variable name="files" select="collection(concat($TOPICS_URI, '?select=*.dita'))"/>

	<xsl:template match="map">
		<dummy/>
		<xsl:for-each select="$files">
			<xsl:result-document href="{concat($OUT_TOPICS_URI, tokenize(base-uri(.), '/')[last()])}">
