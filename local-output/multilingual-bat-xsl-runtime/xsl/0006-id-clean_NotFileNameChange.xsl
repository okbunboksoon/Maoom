<xsl:stylesheet version="2.0" 
	...
	<xsl:variable name="map-title" select="if (normalize-space(/*/title[1]) != '') then normalize-space(/*/title[1]) else replace(string(/*/@mapname), '\.ditamap$', '')"/>
...
-->
	<!-- topic 계열 요소 : 기존 파일명 또는 title 기반 prefix + t0000-CH01 형식 파일명 부여 -->
	<xsl:template match="concept | task | reference">
	    <xsl:variable name="depth" select="count(ancestor::*)"/>
		<xsl:variable name="index" select="count(preceding::concept) + count(preceding::task) + count(preceding::reference)"/>
		<xsl:variable name="chapter-topicref" select="ancestor::topicref[parent::map][1]"/>
		<xsl:variable name="chapter-number" select="if ($chapter-topicref) then format-number(count($chapter-topicref/preceding-sibling::topicref) + 1, '00') else ''"/>
	    <!-- 부모 topicref href -->
