...
if exist "%LANG_DIR%\*.ditamap" (
    ...
    call :REPLACE_SPECIAL_DITA "%LANG_DIR%" "%LANG_NAME%" "%SPECIAL_MERGE_XSL%"
    if errorlevel 1 exit /b 1
...
java -cp "%JAVA_CP%" %SAXON_MAIN% -s:"%TEMPDIR%\14-namespace-removed.xml" -o:"%TEMPDIR%\15-id-cleaned.xml" -xsl:"%XSLDIR%\0006-id-clean_NotFileNameChange.xsl" titleFileNamePrefix=%TITLE_FILE_NAME_PREFIX% langName=%LANG_NAME%
if errorlevel 1 exit /b 1

java -cp "%JAVA_CP%" %SAXON_MAIN% -s:"%TEMPDIR%\15-id-cleaned.xml" -o:"%TEMPDIR%\16-xref-cleaned.xml" -xsl:"%XSLDIR%\0007-xref-clean_NotFileNameChange.xsl" titleFileNamePrefix=%TITLE_FILE_NAME_PREFIX% langName=%LANG_NAME%
if errorlevel 1 exit /b 1
...
java -cp "%JAVA_CP%" %SAXON_MAIN% -s:"%BUILD_SOURCE%" -o:"%TEMPDIR%\21-id-clean-title-prefix.xml" -xsl:"%XSLDIR%\0006-id-clean_NotFileNameChange.xsl" titleFileNamePrefix=Y langName=%LANG_NAME%
if errorlevel 1 exit /b 1

java -cp "%JAVA_CP%" %SAXON_MAIN% -s:"%TEMPDIR%\21-id-clean-title-prefix.xml" -o:"%TEMPDIR%\22-xref-clean-title-prefix.xml" -xsl:"%XSLDIR%\0007-xref-clean_NotFileNameChange.xsl" titleFileNamePrefix=Y langName=%LANG_NAME%
if errorlevel 1 exit /b 1
...
rem 21. topicalize
java -cp "%JAVA_CP%" %SAXON_MAIN% -s:"%BUILD_SOURCE%" -o:"%XSLDIR%\dummy.xml" -xsl:"%XSLDIR%\0210-topicalize.xsl"
if errorlevel 1 exit /b 1

rem 22. make map
java -cp "%JAVA_CP%" %SAXON_MAIN% -s:"%BUILD_SOURCE%" -o:"%TEMPDIR%\tempmap.xml" -xsl:"%XSLDIR%\0220-make-map.xsl"
if errorlevel 1 exit /b 1

rem insert metadata
java -cp "%JAVA_CP%" %SAXON_MAIN% -s:"%TEMPDIR%\tempmap.xml" -o:"%TEMPDIR%\11-metadata_Insert.xml" -xsl:"%XSLDIR%\0225-chapter_metadata_Insert_all.xsl" langName=%LANG_NAME%
if errorlevel 1 exit /b 1

rem 23. map beautify
java -cp "%JAVA_CP%" %SAXON_MAIN% -s:"%TEMPDIR%\11-metadata_Insert.xml" -o:"%XSLDIR%\dummy.xml" -xsl:"%XSLDIR%\0230-map-beautify_meta_all.xsl"
if errorlevel 1 exit /b 1

rem 24. DITA beautify
java -cp "%JAVA_CP%" %SAXON_MAIN% -catalog:"%XSLDIR%\catalog.xml" -s:"%TEMPDIR%\11-metadata_Insert.xml" -o:"%XSLDIR%\dummy.xml" -xsl:"%XSLDIR%\0240-dita-beautify.xsl"
if errorlevel 1 exit /b 1
exit /b 0
